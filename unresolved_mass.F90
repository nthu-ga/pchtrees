! Integral related to the mass fraction in halos below the imposed resolutiom
! limit m_min.
!
! Here gamma_1 (set in module Modified_Merger_Tree) is a parameter involved in
! modifying the merger rate expression away from the Press-Schechter form.
!
...
real function J_UNRESOLVED(z)
    ...
    if (abs(gamma_1).gt.EPS) then ! gamma_1.ne.0
      if (ifirst.eq.0) then ! on the first call tabulate the integral.
        ...
        if (abs(1.0-gamma_1).gt.EPS) then
          ! gamma_1.ne.1
          J_tab(1) = (dz**(1.0-gamma_1))/(1.0-gamma_1)
        else
          ! special case if gamma_1=1
          J_tab(1) = log(dz)
        end if
      ...
      else if (i.ge.NTAB) then
        ! if beyond tabulated region assume the z>>1 analytic approimation that
        ! the integrand is unity and hence J grows linearly with z
        J_UNRESOLVED = J_tab(NTAB) + z - z_tab(NTAB)
      else
        ! linearly interpolate between tabulated values
        h = (z-z_tab(i))*inv_dz
